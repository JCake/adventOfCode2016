const input = `bot 147 gives low to bot 67 and high to bot 71
...
function giveToBot(bot, value) {
    if(botValuesMap[bot]){
        botValuesMap[bot].push(value);
    }
    else {
        botValuesMap[bot] = [value];
    }
}
const updating = [];
instructions.forEach((instruction) => {
    const initialInstruct = instruction.match(/value (\d+) goes to bot (\d+)/);
    if(initialInstruct?.length){
        const value = parseInt(initialInstruct[1]);
        const bot = initialInstruct[2];
        giveToBot(bot, value);
    } else {
        const update = instruction.match(/bot (\d+) gives (low|high) to (output|bot) (\d+) and (low|high) to (output|bot) (\d+)/)
        const firstTransfer = {type: update[3], id: update[4]};
        const secondTransfer = {type: update[6], id: update[7]}
        updating.push({
            sourceBot: update[1],
            lowDest: update[2] === 'low' ? firstTransfer : secondTransfer,
            highDest: update[2] === 'low' ? secondTransfer : firstTransfer
        })
    }
});

const outputsValueMap = {};
while(updating.filter(update => !update.complete)?.length){
    updating.filter(update => {
        return !update.complete && botValuesMap[update.sourceBot]?.length === 2;
    })
    .forEach( update => {
        const botValues = botValuesMap[update.sourceBot];
    
        const lowNum = botValues[0] < botValues[1] ? botValues[0] : botValues[1];
        const lowDest = update.lowDest;
        if(lowDest.type === 'bot'){
            giveToBot(lowDest.id, lowNum, botValuesMap);
        } else {
            outputsValueMap[lowDest.id] = lowNum;
        }
    
        const highNum = botValues[0] < botValues[1] ? botValues[1] : botValues[0];
        const highDest = update.highDest;
        if(highDest.type === 'bot'){
            giveToBot(highDest.id, highNum, botValuesMap);
        } else {
            outputsValueMap[highDest.id] = highNum;
        } 
    
        if(lowNum === 17 && highNum === 61){
            console.log('requested comparison bot: ' + update.sourceBot);
        }
        
        botValuesMap[update.sourceBot] = [];
...
console.log(botValuesMap);
console.log(outputsValueMap);
