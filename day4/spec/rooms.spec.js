describe('triangles', () => {
    ...
    it('finds a real room when checksum matches letter frequency and alphabetical order', () => {
        const roomFinder = new Rooms();
        expect(roomFinder.isReal('aaaaa-bbb-z-y-x-123[abxyz]')).toBeTruthy();
    });

    it('finds a not real room when checksum does not match letter frequency', () => {
        const roomFinder = new Rooms();
        expect(roomFinder.isReal('totally-real-room-200[decoy]')).toBeFalsy();
    });
    
    it('finds a real room when checksum matches letter frequency but is not alphabetical', () => {
        const roomFinder = new Rooms();
        expect(roomFinder.isReal('ot-a-real-room-404[oarel]')).toBeTruthy();
    });

    it('finds not a real room when checksum has ties in frequency not in alphabetical order', () => {
        const roomFinder = new Rooms();
        expect(roomFinder.isReal('aaaaa-bbb-z-y-x-123[abzxy]')).toBeFalsy();
    });

    it('finds sum of real room sector ids', () => {
        const roomFinder = new Rooms();
        expect(roomFinder.sumOfRealRoomSectorIds(`aaaaa-bbb-z-y-x-123[abxyz]
        a-b-c-d-e-f-g-h-987[abcde]
        not-a-real-room-404[oarel]
        totally-real-room-200[decoy]`)).toEqual(1514);
    });

    const puzzleInput = `hqcfqwydw-fbqijys-whqii-huiuqhsx-660[qhiwf]
...
aoubshwq-pibbm-kcfygvcd-740[wnucy]`;
    ...
    it('finds sum of real room sector ids for puzzleInput', () => {
        const roomFinder = new Rooms();
        expect(roomFinder.sumOfRealRoomSectorIds(puzzleInput)).toEqual(245102);
    });

    it('finds sector id hiding toys', () => {
        const roomFinder = new Rooms();
        expect(roomFinder.namesOfRealRooms(puzzleInput)).toEqual(324);
    });
