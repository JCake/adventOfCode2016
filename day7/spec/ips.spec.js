describe('IP address classification', () => {
    ...
    it('finds IPs that support TLS', () => {
        const ipClassifier = new IPClassifier();
        expect(ipClassifier.supportsTLS('abba[mnop]qrst')).toBeTruthy();
        expect(ipClassifier.supportsTLS('abcd[bddb]xyyx')).toBeFalsy();
        expect(ipClassifier.supportsTLS('aaaa[qwer]tyui')).toBeFalsy();
        expect(ipClassifier.supportsTLS('ioxxoj[asdfgh]zxcvbn')).toBeTruthy();
    });

    it('finds a count of IPs that support TLS', () => {
        const ipClassifier = new IPClassifier();
        const sampleInput = `abba[mnop]qrst
abcd[bddb]xyyx
aaaa[qwer]tyui
ioxxoj[asdfgh]zxcvbn`
        expect(ipClassifier.numberSupportingTLS(sampleInput)).toEqual(2);
    });

    it('looks in multiple subnets', () => {
        const ipClassifier = new IPClassifier();
        expect(ipClassifier.supportsTLS('[mnop]abba[mnop]qrst')).toBeTruthy();
        expect(ipClassifier.supportsTLS('[mnop]abba[abba]qrst')).toBeFalsy();
    });

    const realPuzzleInput =
...
bwzsacxgqkbjycgfw[dbnligvrmqscasutn]rbgybqqsgjvlonkut`;
    ...
    it('finds a count of IPs that support TLS for real input', () => {
        const ipClassifier = new IPClassifier();
        expect(ipClassifier.numberSupportingTLS(realPuzzleInput)).toEqual(118);
    });

    describe('part 2', () => {
        it('finds whether an address supports SSL', () => {
            const ipClassifier = new IPClassifier();
            expect(ipClassifier.supportsSSL('aba[bab]xyz')).toBeTruthy();
            expect(ipClassifier.supportsSSL('xyx[xyx]xyx')).toBeFalsy();
            expect(ipClassifier.supportsSSL('aaa[kek]eke')).toBeTruthy();
            expect(ipClassifier.supportsSSL('zazbz[bzb]cdb')).toBeTruthy();
        });

        it('excludes both matches being inside brackets', () => {
            const ipClassifier = new IPClassifier();
            expect(ipClassifier.supportsSSL('xyx[bababa]xyz')).toBeFalsy();
        });

        it('handles looking for matches between multiple bracketed parts', () => {
            const ipClassifier = new IPClassifier();
            expect(ipClassifier.supportsSSL('aaa[kek]eke[xyz]zzz')).toBeTruthy();
            expect(ipClassifier.supportsSSL('aaa[kek]eke[xyz]')).toBeTruthy();
            expect(ipClassifier.supportsSSL('aaa[kek]kek[xyz]zzz')).toBeFalsy();
            expect(ipClassifier.supportsSSL('aaa[kek]syz[eke]zzz')).toBeFalsy();
        });

        it('counts ssl matches', () => {
            const ipClassifier = new IPClassifier();
            expect(ipClassifier.numberSupportingSSL(
`aba[bab]xyz
xyx[xyx]xyx
aaa[kek]eke
zazbz[bzb]cdb
xyx[bababa]xyz`)).toEqual(3);

        });

                it('counts ssl matches for real input', () => {
            const ipClassifier = new IPClassifier();
            expect(ipClassifier.numberSupportingSSL(realPuzzleInput)).toEqual(260);
